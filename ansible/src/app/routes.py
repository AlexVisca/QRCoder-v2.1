...
def get_data(filename="username.txt"):
    data = {
        "success": False
    }
    try:
    ...
    name = lines[0].strip()
    username_id = lines[1].strip()
    username_id_matches = re.match("A\d{8}", username_id)

    if not name or not username_id_matches:
        data.update(msg=f"The contents of {filename} are incorrect.")
        return data

    data.update(success=True)
    data.update(username_id=username_id, username=name)
    return data
...
def qrcode():
    ...
    username_id = values["username_id"]
    ...
    params = {
        "size": "200x200",
        "data": "\n".join([name, username_id, cur_date]),
        "format": "png",
    }
